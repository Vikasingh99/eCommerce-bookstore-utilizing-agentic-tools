...
/**
 * Get a seeded product by title.
 * Throws if the DB has not been seeded — run `node prisma/seed.js` first.
 * Never creates records in the real product table.
 */
async function getTestProduct(title = 'Atomic Habits') {
  const product = await prisma.product.findFirst({ where: { title } });
  if (!product) {
    throw new Error(
      `Test product "${title}" not found. Run: node prisma/seed.js`
    );
  }
  return product;
}
